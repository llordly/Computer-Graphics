...
def draw_curve(t, pointlist):
    a = (1 - t) ** 3 * pointlist[0]
    b = (1 - t) ** 2 * t * pointlist[1]
    c = 2 * (1 - t) ** 2 * t * pointlist[1]
    d = 2 * (1 - t) * (t ** 2) * pointlist[2]
    e = (1 - t) * (t ** 2) * pointlist[2]
    f = t ** 3 * pointlist[3]
    glVertex2fv(a + b + c + d + e + f)
...
def render():
    ...
    for t in np.arange(0, 1, .01):
        glVertex2fv(Lerp(t, p0, p1))
    for t in np.arange(0, 1, .01):
        glVertex2fv(Lerp(t, p1, p2))
    for t in np.arange(0, 1, .01):
        glVertex2fv(Lerp(t, p2, p3))
    for t in np.arange(0, 1, .01):
        glVertex2fv(Lerp(t, p3, p0))
